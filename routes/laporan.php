  ...
  post('/laporan/view', function() {
      ...
      if ($value->is_approve == 0) {
        $model[$key]['status'] = "Belum Disetujui";
      }elseif ($value->is_approve == 1 && $value->is_naik == 0 && $value->is_turun == 0) {
        $model[$key]['status'] = "Disetujui";
      }elseif ($value->is_approve == 1 && $value->is_naik == 1 && $value->is_turun == 0) {
        $model[$key]['status'] = "Naik";
      }elseif ($value->is_approve == 1 && $value->is_naik == 1 && $value->is_turun == 1) {
        $model[$key]['status'] = "Turun";
      }
