...
function createImg($path, $filename, $id, $proporsional = false) {
    ...
    $medium = img_path() . $path . $id . '-350x350-' . $newFileName;
    ...
    smart_resize_image($file, $medium, 350, 350, $proporsional, 80);
    smart_resize_image($file, $big, 700, 700, $proporsional, 80);

    unlink(img_path() . $path . $filename);

    return array('big' => $id . '-700x700-' . $newFileName, 'small' => $id . '-150x150-' . $newFileName);
}
...
function pagination($item_count, $limit, $cur_page, $link) {
    if ($limit > 0 && $item_count > 0) {
        ...
        $paginate = '<div class="col-md-12" style="text-align:center"><ul class="pagination pagination-blog">';
        if ($page_count != 1) {
            if ($cur_page == 1) {
